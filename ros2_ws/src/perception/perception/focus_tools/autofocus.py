...
class FrameStorage:
    ...
    def update_frame(self, frame):
        """Called by the subscription callback whenever a new frame arrives."""
        with self.lock:
            self.current_frame = frame
            self.frame_id += 1       # increment ID so getFrame() knows a new frame arrived
            self.new_frame_condition.notify_all()

    def getFrame(self, timeout=1000):
        """
        Blocks until a new frame arrives or timeout elapses (in ms).
        Returns the latest frame or None if no new frame arrived within the timeout.
        """
        # Convert ms to seconds
        timeout_sec = timeout / 1000.0

        with self.lock:
            # Record the frame_id so we know if a new frame arrived
            start_id = self.frame_id
            start_time = time.time()

            # While there's no new frame, wait up to 'timeout_sec'
            while self.frame_id == start_id:
                elapsed = time.time() - start_time
                remaining = timeout_sec - elapsed
                if remaining <= 0:
                    # Timed out, no new frame
                    return None

                # Wait for a new frame to arrive or the remainder of the timeout
                self.new_frame_condition.wait(timeout=remaining)

            # If we get here, self.frame_id changed => a new frame arrived
