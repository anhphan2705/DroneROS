...
class RectificationNode(Node):
    ...
    def apply_latest_calibration(self, pair_id):
        """ Load and apply the latest calibration file. """
        ...
        # Store precomputed rectification maps
        self.rectification_maps[pair_id] = {
            "left": (cv2.UMat(stereo_map_left_x), cv2.UMat(stereo_map_left_y)),  # Hardware acceleration
            "right": (cv2.UMat(stereo_map_right_x), cv2.UMat(stereo_map_right_y)),
        }
    ...
    def image_callback(self, msg, topic):
        ...
        # Apply rectification using hardware-accelerated `cv2.remap()`
        rectified_image = cv2.remap(cv_image, map_x, map_y, interpolation=cv2.INTER_NEAREST).get()  # Faster interpolation

        # Convert and publish rectified image
        rect_msg = self.br.cv2_to_imgmsg(rectified_image, encoding='bgr8')
        rect_msg.header = msg.header  # Preserve original timestamp
